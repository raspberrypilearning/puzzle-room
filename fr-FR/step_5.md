...
```blocks3
définir aller à la position
aller à (position aléatoire v)
répéter jusqu'à <<touching (port v) ?> et <not <touching color (#69B486) ?>>
+ mettre l'effet [fantôme v] à (100) //Cacher le sprite
aller à (position aléatoire v)
fin
+ effacer les effets graphiques
```
